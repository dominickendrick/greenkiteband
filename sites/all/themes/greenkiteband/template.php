...
function greenkiteband_links($links, $attributes = array('class' => 'links')) {
  global $language;
  $output = '';
...
function greenkiteband_breadcrumb($breadcrumb) {
...
function greenkiteband_aggregator_block_item($item, $feed = 0) {
...
function greenkiteband_preprocess_node(&$vars, $hook) {
 ...
 drupal_add_js(drupal_get_path('theme', 'greenkiteband').'/thumbnails.js', 'theme');
 drupal_add_js(drupal_get_path('theme', 'greenkiteband').'/supersleight.plugin.js', 'theme'); 
 drupal_add_js(drupal_get_path('theme', 'greenkiteband').'/png.js', 'theme');
...
$function = 'greenkiteband_preprocess_node'.'_'. $vars['node']->type;
  if (function_exists($function)) {
   $function($vars);
  }
/*  greenkiteband_preprocess_node_video($vars);
  greenkiteband_preprocess_node_photo($vars);
  greenkiteband_preprocess_node_audio($vars); */
}
...
function greenkiteband_preprocess_node_video(&$vars) {
...
function greenkiteband_preprocess_node_photo(&$vars) {
...
function greenkiteband_preprocess_node_audio(&$vars) {
  drupal_add_js(drupal_get_path('theme', 'greenkiteband').'/audio-player/audio-player.js', 'theme');
...
function greenkiteband_preprocess_node_terms_conditions(&$vars) {
 ...
 *
 */
 function greenkiteband_service_links_build_link($text, $url, $title, $image, $nodelink) {
  global $base_path;
